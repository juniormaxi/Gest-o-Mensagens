...
type TokenPayload = { sub: string; role: UserRole; name: string; email: string };
...
export const requireRole = (...roles: UserRole[]): RequestHandler => (req, _res, next) => req.user && roles.includes(req.user.role) ? next() : next(new AppError(403, 'Acesso negado'));
